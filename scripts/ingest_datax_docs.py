"""DataX 知识库一键灌库：构建官方双语语料 → MyRag collection 隔离索引。

流程：
  1. 调用 build_datax_corpus.py 从 DataX 官方仓库（GitHub/Gitee 镜像）提取
     doc/*.md，清洗为中英双语结构化 JSONL 语料（含踩坑经验）；
  2. 调用 MyRag 的 ingest.py --collection datax_docs 灌库到独立 ES 索引
...
MYRAG_ROOT = Path(r"F:\PycharmProjects\pyspark\MyRag")
...
def main() -> None:
    ...
    # 2) 灌库（MyRag 原生管线：分块 + 本地 embedding + 双路召回索引）
    ingest_cmd = [sys.executable, "ingest.py", "--collection", COLLECTION]
    if args.no_rebuild:
        ingest_cmd.append("--no-rebuild")
    run(ingest_cmd, MYRAG_ROOT)
