...
from ..tools import (
    ...
    retry_failed_task, kill_datax_process_tree,
)
...
class OpsDiagnosisAgent(BaseAgent):
    ...
    def run(self, state: DataIntegrationState) -> DataIntegrationState:
        ...
        # 2. LLM 诊断（失败时规则兜底）
        diagnosis = self._llm_diagnose(task_id, task, error, log_tail, rag_hits)
    ...
    def _llm_diagnose(
        ...
        return self._fallback_diagnose(task_id, task, error, rag_hits)

    @staticmethod
    def _fallback_diagnose(task_id: str, task: dict, error: str, rag_hits: list[dict]) -> dict:
        """规则兜底：以错误信息 + 事故库命中为核心。"""
...
class OpsRecordAgent(BaseAgent):
    ...
    def run(self, state: DataIntegrationState) -> DataIntegrationState:
        ...
        if os.getenv("OPS_AUTO_RECORD", "true").strip().lower() == "false":
            ...
            summary = f"已沉淀事故记录: {incident_id}" if incident_id else "跳过沉淀（相似未解决事故已存在或无有效信息）"
    ...
    def _record_incident(
        ...
        title = f"{task_id} 失败诊断: {(root_cause or error)[:60]}"
        ...
        for rec in _load_records(store):
            if rec.get("title") == title and rec.get("status") in ("open", "investigating"):
                return None

        record = {
            "incident_id": f"incident-{hashlib.md5(title.encode()).hexdigest()[:8]}",
            "title": title,
